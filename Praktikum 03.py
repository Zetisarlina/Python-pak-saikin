bukuclass Buku:
def init(self, judul, penulis, genre, status):
self.judul = judul
...
def pinjam_buku(self, judul, anggota):
    ...
    print(f"Buku dengan judul '{judul}' tidak ditemukan.")
class Anggota:
def init(self, nama, ID, alamat=None, nomor_telepon=None):
self.nama = nama
self.ID = ID
self.alamat = alamat
...
def init(self, id_buku, id_anggota, tanggal_pinjam, tanggal_kembali):
